...
public class JettySocks5Proxy extends Proxy {

    private static final Logger log = LoggerFactory.getLogger(JettySocks5Proxy.class);

    private String username;
    private String password;
  
    public JettySocks5Proxy(String host, int port) {
        this(host, port, null, null);
    }
    
    public JettySocks5Proxy(String host, int port, String username, String password) {
      super(new Address(host, port), false);
    ...
    private static class Socks5ProxyConnection extends AbstractConnection implements Callback {
      ...
      public void failed(Throwable x) {
        this.close();
        Promise<Connection> promise = (Promise)this.context.get("http.connection.promise");
        promise.failed(x);
      }
